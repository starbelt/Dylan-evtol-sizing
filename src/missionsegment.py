...
class MissionSegment:
    def __init__(self, path_to_json: str, segment_type: str):
        ...
        self.power_draw_kw: float | None = None
        self.c_rate: float | None = None
        self.duration_s: float | None = None
        self.energy_kwh: float | None = None

    @abc.abstractmethod
    def _load_static_params_from_json(self, path_to_json: str):
        """Loads segment-specific static parameters from JSON."""
        raise NotImplementedError

    @abc.abstractmethod
    def calculate_performance(self, aircraft: Aircraft):
        """Calculates power, duration, C-rate, and energy for the segment."""
        raise NotImplementedError

    def get_results(self) -> dict:
        """Returns the calculated results for the segment."""
        return {
...
class ClimbSegment(MissionSegment):
    ...
    def _load_static_params_from_json(self, path_to_json):
        ...
        self.num_steps = data.get("num_integration_steps", 40)
    ...
    def calculate_performance(self, aircraft: Aircraft):
        ...
        for i in range(1, self.num_steps + 1): # Loop from step 1 to N
            ...
            if v_end_mps > v_start_mps: # Check if accelerating
            ...
            # Accumulate energy using Trapezoidal rule (AvgPower * dt)
...
class ShorterRangeMission(Mission):
    """
    A specialized Mission class for simulating a shorter range,
    typically used for ABU optimization baseline.
    """
    def __init__(self, aircraft: Aircraft, base_mission_segments: list[MissionSegment], shorter_range_km: float):
        ...
        Args:
            aircraft (Aircraft): The aircraft instance.
            base_mission_segments (list[MissionSegment]): The original list of mission segments.
            shorter_range_km (float): The target range for the cruise segment in this shorter mission.
        """
        # Deep copy the base segments to modify them locally
        ...
        for seg in modified_segments:
            if isinstance(seg, CruiseSegment):
                ...
                break # Assume only one main cruise segment needs modification
        ...
        self.shorter_range_km = shorter_range_km # Store for reference if needed
...
def calc_power_at_speed_climbing_k_W(
    ...
    lift = q * wing_ref_area_m2 * cl_req
